...
每次招聘请求通过数据库读取当前运行时 Base URL 和模型；未保存运行时配置时使用服务端环境默认值。管理员保存的新配置无需重启 API 即可供后续请求使用。
...
运行时配置只保存 Base URL 和单一当前模型，不构成多模型路由或聚合。模型候选项由后端使用服务端 Key 调用受控上游 `/v1/models` 动态发现。
